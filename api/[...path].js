export default async function handler(req, res) {
  ...
  // Remove the /api/ prefix from pathStr
  const apiPath = pathStr;

  if (apiPath.startsWith('weixin-long')) {
    targetUrl = `https://long.open.weixin.qq.com/${apiPath.replace(/^weixin-long\/?/, '')}`;
  ...
  } else if (apiPath.startsWith('weixin')) {
    targetUrl = `https://open.weixin.qq.com/${apiPath.replace(/^weixin\/?/, '')}`;
    headers['User-Agent'] = 'Mozilla/5.0 (Linux; Android 7.0; Mi-4c Build/NRD90M; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/53.0.2785.49 Mobile MQQBrowser/6.2 TBS/043632 Safari/537.36 MicroMessenger/6.6.1.1220(0x26060135) NetType/WIFI Language/zh_CN';
    headers['Accept'] = 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8';
    headers['Referer'] = 'https://open.weixin.qq.com/';
  } else if (apiPath.startsWith('hortor')) {
    targetUrl = `https://comb-platform.hortorgames.com/${apiPath.replace(/^hortor\/?/, '')}`;
    ...
    console.log('Invalid API path:', apiPath);
    return res.status(400).json({ error: 'Invalid API path', received: apiPath });
  }
